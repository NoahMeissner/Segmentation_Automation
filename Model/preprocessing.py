    ...
    if filename.endswith('.csv'):
        ...
        with open(csv_file, 'r') as csv_file, open(os.path.join(input_directory, txt_filename), 'w') as txt_file:
            ...
            for row in csv_reader:
                ...
                # kill the garbage
                filtered_values = [cell for cell in filtered_values if not (has_hashtag and cell.strip().isdigit())]
...
source_folder = os.path.join(parent_folder, "Images")
destination_subfolders = [os.path.join(subfolder, folder) for subfolder in subfolders for folder in ["images", "labels"]]

# Create destination subfolders if they don't exist
for folder in destination_subfolders:
...
for i, (start, end) in enumerate(zip([0] + split_points, split_points)):
    for file_name in jpg_files[start:end]:
        ...
        destination_path = os.path.join(destination_subfolders[i * 2], file_name)
        shutil.move(source_path, destination_path)

    for file_name in txt_files[start:end]:
        source_path = os.path.join(source_folder, file_name)
        destination_path = os.path.join(destination_subfolders[i * 2 + 1], file_name)
        shutil.move(source_path, destination_path)
